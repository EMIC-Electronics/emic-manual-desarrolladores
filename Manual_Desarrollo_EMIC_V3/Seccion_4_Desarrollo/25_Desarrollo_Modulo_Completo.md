...
## 25.9 Nota sobre Creación de Nuevos SDKs

> **⚠️ IMPORTANTE:** Este capítulo trata sobre la creación de módulos **dentro de un SDK existente**. Si estás creando un **nuevo SDK completo** (por ejemplo, para una nueva familia de microcontroladores), debes incluir adicionalmente:

```
NUEVO_SDK/
├── .emic/                          ← OBLIGATORIO para nuevos SDKs
...
El archivo `.emic/repository.json` es **obligatorio** para que el sistema EMIC reconozca el directorio como un SDK válido y pueda indexarlo en el catálogo de SDKs disponibles.

**Ver Capítulo 05b** para información completa sobre la estructura de la carpeta `.emic` y el formato del archivo `repository.json`.
